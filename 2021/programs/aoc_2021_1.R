...
# read in data
my_data <- read.table(file.path("raw", "aoc_2021_1_input.txt"), blank.lines.skip = F) %>% 
  rename(measurement=V1)

# calculate differences from one row to the next
diff <- as_tibble(diff(my_data$measurement, lag=1))

# filter for those greater than 0
gt0 <- diff %>% 
  filter(value > 0)
...
# sum. So, compare A with B, then compare B with C, then C with D, and so on. Stop when there aren't enough measurements left 
...
# calculate sums of 3 meas sliding window
library(zoo)
sum3 <- as_tibble(rollsumr(my_data$measurement, k = 3, fill = NA))

# calculate differences from one row to the next
diffsum3 <- as_tibble(diff(sum3$value, lag=1))
